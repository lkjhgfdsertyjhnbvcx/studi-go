...
export default function MyStudiosPage() {
    ...
    if (loading) return <div className="min-h-screen bg-background text-foreground p-20">読み込み中...</div>;
    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-6 md:p-20 font-sans relative">
            ...
            <div className="absolute top-6 left-6 md:top-10 md:left-10 z-10 flex items-center gap-8">
                <Link href="/studios">
                    <Image
                        ...
                        width={200}
                        height={60}
                        className="h-12 w-auto object-contain dark:invert hover:opacity-80 transition-opacity"
                        priority
                    />
                </Link>
            </div>
            ...
            <header className="mb-12 text-center pt-10">
                <h1 className="text-4xl md:text-7xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-600 mb-6 uppercase italic">
                    マインスタジオ
                </h1>
                ...
                {studios.length > 0 ? studios.map((studio, i) => {
                    return (
                        <Link href={`/studios/${studio.id}`} key={studio.id} className="group cursor-pointer">
                            <div className="relative h-64 bg-card border border-border rounded-2xl overflow-hidden transition-all duration-500 group-hover:scale-105 group-hover:border-pink-500/50 group-hover:shadow-[0_0_50px_rgba(236,72,153,0.3)]">
                                ...
                                <div className="absolute inset-0 p-8 flex flex-col justify-between z-10">
                                    <div>
                                        ...
                                        <h2 className="text-3xl font-bold text-white mb-1 group-hover:text-pink-400 transition-colors drop-shadow-lg">
                                            {studio.storeName}
                                        </h2>
                                    ...
                                    <div className="flex justify-between items-center border-t border-white/10 dark:border-white/10 pt-4">
                                        <div className="text-sm">
                                            <span className="block text-gray-400 text-xs">部屋数</span>
