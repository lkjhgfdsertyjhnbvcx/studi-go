...
export default function AdsPage() {
    ...
    return (
        <div className="p-8 max-w-2xl mx-auto space-y-8">
            <div>
                <h1 className="text-2xl font-bold text-foreground">広告・アフィリエイト管理</h1>
                <p className="text-muted-foreground text-sm mt-1">決済完了画面などに表示する広告を設定します</p>
            </div>
            <div className="bg-card border border-border rounded-2xl p-6">
                <h2 className="font-bold text-foreground mb-4">広告表示</h2>
            ...
            {s.enabled && (<>
                <div className="bg-card border border-border rounded-2xl p-6 space-y-3">
                    <h2 className="font-bold text-foreground">広告の種類</h2>
                ...
                {s.provider==="adsense" && (
                    <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                        <h2 className="font-bold text-foreground">Google AdSense設定</h2>
                        <div><label className="text-xs font-bold text-muted-foreground">クライアントID</label>
                        ...
                        <div><label className="text-xs font-bold text-muted-foreground">広告スロットID</label>
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 text-xs text-blue-400 space-y-1">
                            <p className="font-bold">AdSense設定手順</p>
                ...
                {s.provider==="affiliate" && (
                    <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="font-bold text-foreground">アフィリエイト設定</h2>
                            <button onClick={addAff} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold rounded-xl transition-all">+ 追加</button>
                        </div>
                        ...
                        {s.affiliates.map((aff,i)=>(
                            <div key={aff.id} className="border border-border rounded-xl p-4 space-y-3">
                                <div className="flex items-center justify-between">
                                    <p className="text-xs font-bold text-muted-foreground">広告 {i+1}</p>
                ...
                {s.provider==="custom" && (
                    <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                        <h2 className="font-bold text-foreground">カスタム広告HTML</h2>
                ...
                <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                    <h2 className="font-bold text-foreground">表示場所</h2>
            ...
            <button onClick={save} className={`w-full py-4 rounded-2xl font-bold text-sm transition-all ${saved?"bg-green-500 text-white":"bg-purple-600 hover:bg-purple-500 text-white"}`}>
                {saved?"✓ 保存しました":"保存する"}
            </button>
        </div>
    );
