    ...
    if (data.isPersonalPractice) {
        ...
        if (settings.reservationWindowType === 'days') {
            ...
            if (diffDays > settings.reservationWindowValue) {
                return {
                    success: false,
                    message: `個人練習は利用日の${settings.reservationWindowValue}日前から予約可能です。`
                };
            }
        } else {
            ...
            if (diffHours > settings.reservationWindowValue) {
                return {
                    success: false,
                    message: `個人練習は利用の${settings.reservationWindowValue}時間前から予約可能です。`
                };
            }
        }
        // People check
        if (userCount > settings.maxPeople) {
            return {
                success: false,
                message: `個人練習は最大${settings.maxPeople}名までです。`
            };
        }
    }
    // 2. Calculate Price
...
async function fetchMyBookings() {
    ...
    const { getAllStudios } = await __turbopack_context__.A("[project]/music-studio-app/src/lib/db-studio.ts [app-rsc] (ecmascript, async loader)");
    ...
    if (!user) return [];
    const studios = getAllStudios();
    const studioMap = new Map(studios.map((s)=>[
            s.id,
            {
                name: s.storeName,
    ...
    let payments = [];
    try {
        const pPath = path.join(process.cwd(), "data", "payments.json");
        if (fs.existsSync(pPath)) {
            payments = JSON.parse(fs.readFileSync(pPath, "utf-8"));
        }
    } catch (e) {
        console.error("Failed to load payments for status check", e);
    }
    const paymentStatusMap = new Map(payments.map((p)=>[
            p.bookingId,
            p.status
        ]));
    return getBookings().filter((b)=>b.userId === user.id).sort((a, b)=>new Date(b.date).getTime() - new Date(a.date).getTime()).map((b)=>{
...
"[project]/music-studio-app/.next-internal/server/app/page/actions.js { ACTIONS_MODULE0 => \"[project]/music-studio-app/src/actions/login.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE1 => \"[project]/music-studio-app/src/actions/booking.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE2 => \"[project]/music-studio-app/src/actions/schedule.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
"[project]/music-studio-app/.next-internal/server/app/page/actions.js { ACTIONS_MODULE0 => \"[project]/music-studio-app/src/actions/login.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE1 => \"[project]/music-studio-app/src/actions/booking.ts [app-rsc] (ecmascript)\", ACTIONS_MODULE2 => \"[project]/music-studio-app/src/actions/schedule.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__) => {
"use strict";
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a5d7c50b._.js.map